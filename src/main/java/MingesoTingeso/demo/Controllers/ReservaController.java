...
public class ReservaController {
    ...
    public List<HashMap<String, String>> update(@PathVariable int codigoReserva, @RequestBody Map<String, Object> jsonData) throws ParseException {
        ...
        SimpleDateFormat formatter = new SimpleDateFormat("yyyy-MM-dd");
        Cliente cliente = clienteRepository.findClienteByRut(Integer.parseInt(jsonData.get("rut").toString()));
        Usuario usuario = usuarioRepository.findUsuarioByRutUsuario(reservaU.getUsuario().getRutUsuario());
        Long idUsuario = usuario.getIdUsuario();
        Usuario usuario2= usuarioRepository.findUsuarioByIdUser(idUsuario);
        Cliente editar= reservaU.getCliente();
        if (cliente.getRut() != reservaU.getCliente().getRut()){
            editar.setRut(cliente.getRut());
        ...
        Long idReserva = (long)reservaU.getIdReserva();

        //ReservaHabitacion rh = resHabRepository.findReservaHabitacionBy Reserva(idReserva);
        ...
        map.put("status", "201");
        map.put("idResertva:",  idReserva.toString());
        map.put("message", "OK");
        result.add(map);
        return result;
